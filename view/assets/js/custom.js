...
(function ($) {
  ...
  function neoncart_quantity() {
    $(document).on(
      "click",
      ".quantity_input .input_number_decrement, .quantity_input .input_number_increment",
      function (e) {
        var target = $(e.target),
        ...
        if (qty.length) {
          ...
          function updateinput_number(inputNumber) {
            // Lấy giá trị mới của input_number
            var quantity = parseInt(inputNumber.val());

            console.log(quantity);
            return quantity;
          }
          var quantity = updateinput_number(qty);

          var xhr = new XMLHttpRequest();
          var url = "soluongkc.php"; // Địa chỉ URL của file xử lý PHP
          var params = "quantity=" + parseInt(quantity);
          ...
          xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
              ...
              console.log("SL sản phẩm biến thể: " + quantity);
            }
          };

          xhr.send(params);
          function updateTotalPrice(inputNumber) {
            // Kiểm tra xem có ít nhất một phần tử .total_price không
            if ($(".total_price").length > 0) {
              // Lấy giá trị mới của input_number
              var quantity = parseInt(inputNumber.val());
              console.log("quantity", quantity);

              // Lấy giá trị ban đầu của item_price
              var priceElement = inputNumber
                .closest("td")
                .siblings(".total_price");
              console.log("priceElement", priceElement);

              var initialPrice = parseInt(
                $(".price_text").attr("data-initial-price")
              );

              console.log("initialPrice", initialPrice);

              // Tính toán giá trị mới của item_price
              var newPrice = initialPrice * quantity;
              console.log("newPrice", newPrice);
              // Cập nhật giá trị của item_price
              $(".total_price").text(newPrice + " VNĐ");
            }
          }
  ...
  function neoncart_quantity1() {
    $(document).on(
      "click",
      ".quantity_input1 .input_number_decrement, .quantity_input1 .input_number_increment",
      function (e) {
        var target = $(e.target),
          qty = target.closest(".quantity_input1").find("input.input_number"),
          min,
          max,
          step,
          value;
        ...
        if (qty.length) {
          ...
          updateItemPrice(qty);
          // qty.val(value).change();
          // Gọi hàm cập nhật giá trị item_price sau khi thay đổi giá trị input_number
        }
      }
    );
  }
  neoncart_quantity1();
  // update giasp
  // Định nghĩa hàm tính toán giá trị mới của item_price
  function updateItemPrice(inputNumber) {
    // Lấy giá trị mới của input_number
    var quantity = parseInt(inputNumber.val());

    // Lấy giá trị ban đầu của item_price
    var priceElement = inputNumber.closest("li").find(".item_price");
    var initialPrice = parseInt(priceElement.attr("data-initial-price"));

    // Tính toán giá trị mới của item_price
    var newPrice = initialPrice * quantity;

    // Cập nhật giá trị của item_price
    priceElement.text(newPrice + " VNĐ");
  }

  // Thêm sự kiện change cho input_number để gọi hàm updateItemPrice khi giá trị thay đổi
  // function handleQuantityChanges() {
  //   $(document).on("change", ".quantity_input1 .input_number", function () {
  //     updateItemPrice($(this));
  //   });
  // }
  // // Gọi hàm handleQuantityChanges khi trang đã tải hoàn toàn
  ...
  mouse_scroll_slide.on("wheel", function (e) {
    slick_handle_wheel_event_debounced(
      e.originalEvent,
      mouse_scroll_slide,
      mouse_scroll_slide_is_animating
    );
  });
